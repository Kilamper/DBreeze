...
const Layout = () => {
  ...
  return (
    <div className="flex h-screen">
      {/* Barra lateral fija */}
      <nav className="w-56 bg-gray-900 text-white p-5 border-r border-gray-800 flex flex-col">
          ...
          <ul className="mt-2">
            {connections.map((connection, index) => (
              <li key={index} className="mb-2">
                <div
                  className="cursor-pointer flex items-center gap-x-2 hover:bg-slate-700 rounded-md px-2 py-1.5 text-gray-300 hover:text-white"
                  onClick={() => {
                    const updatedConnections = [...connections];
                    updatedConnections[index].isExpanded = !updatedConnections[index].isExpanded;
                    setConnections(updatedConnections);
                  }}
                >
                  {
                    !connection.isExpanded ? <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
        ...
        <Button variant="primary" className="mt-auto gap-2" onClick={signOut}>Cerrar sesión</Button>
        <div className="flex items-center justify-between mt-2 py-2 px-1">
          <div className="flex items-center gap-x-2">
            <img src={"https://ui-avatars.com/api/?name=" + userName + "&background=random"} className="w-8 h-8 rounded-full"></img>
